...
const Criteria1 = (props) => {
    ...
    const {submitCriteria,getUser,setCriteria,criteria, status, modal} = context
    ...
    const handleClick = (e)=>{
        ...
        if(status==="submitted"){history.push('/allCriteria')}
        setCriteria(null)
        localStorage.setItem("recent",JSON.stringify(criteria))
        
    }

    useEffect(() => {
        localStorage.getItem('token')==='null'&&history.push('/login')
        (!location.state&&localStorage.removeItem("recent"));
          window.scrollTo(0,0)
    ...
    return (
        ...
        <div className="bg-white items-stretch flex justify-center  pt-16 bg-gray-50">
